...
local excute_cmd = function(cmd)
	local ok,data,err = pcall(function()
		...
		if not t then
			return nil,'excute command failed'
		end
	...
	if not ok then
		print('[ERROR] cmd = ' .. cmd .. '  error = ' .. data)
	end
    return data
end
...
if not _M.orpath then
	_M.orpath = _M.myorpath .. '/openresty'
	excute_cmd('chmod 777 ' .. _M.orpath .. '/bin/openresty')
end
--openresty的执行文件的路径
...
_M.execute = excute_cmd

--判定文件是否存在
_M.exist = function(path)
  return tonumber(excute_cmd('test -e ' .. path .. ' && echo 1')) == 1
end

--如果文件存在则删除
_M.remove = function(file)
	local rm_exe = 'test -e ' .. file .. ' && rm ' .. file
	excute_cmd(rm_exe)
end

--创建一个软连接指向目标文件
_M.link = function(dest,link,force)
	if force then
		_M.remove(link)
	end

	if not _M.exist(link) then
...
_M.copy_file = function(file,path)
	...
	_M.execute('cp ' .. file .. ' ' .. path .. '/' .. filename)
end
