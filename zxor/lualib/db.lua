...
for dbname,opts in pairs(config.db or {}) do
	if opts.type == 'mysql' then
		if not opts.user and opts.username then
	...
	elseif opts.type == 'memcached' then
		if not opts.host or not opts.port then
			log_err('Memcached configuration has wrong parameters. memcached name = ',dbname)
		else
			_M[dbname] = memcached:new(opts)
		end
	end
end
