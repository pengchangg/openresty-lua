...
local function fexists(td, elementName)
    if td == nil then
        return false
    end

    if td[elementName] == nil then
        return fexists(getmetatable(td), elementName)
    else
        return true
    end
end
...
local function err(self, errMsg, pos)
    if self.options.errorHandler then
        return self.options.errorHandler(errMsg,pos)
    end
	
	return (errMsg or 'unkonwn') .. ' at ' .. (pos or 0)
end
...
local function parseXmlDeclaration(self, xml, f)
    ...
    if not f.match then 
        return false,err(self, self._errstr.declErr, f.pos)
    end 

    if f.match ~= 1 then
        -- Must be at start of doc if present
        return false,err(self, self._errstr.declStartErr, f.pos)
    end
    ...
    if tag.attrs and tag.attrs.version == nil then
        return false,err(self, self._errstr.declAttrErr, f.pos)
    end
...
local function parseXmlProcessingInstruction(self, xml, f)
    ...
    f.match, f.endMatch, f.text = string.find(xml, self._PI, f.pos)
    if not f.match then 
        return false,err(self, self._errstr.piErr, f.pos)
    end 
...
local function parseComment(self, xml, f)
    ...
    if not f.match then 
        return false,err(self, self._errstr.commentErr, f.pos)
    end 
...
local function parseDtd(self, xml, f)
    ...
    if not f.match then 
        return false,err(self, self._errstr.dtdErr, f.pos)
    end 
...
local function parseCdata(self, xml, f)
    ...
    if not f.match then 
        return false,err(self, self._errstr.cdataErr, f.pos)
    end 
...
local function parseNormalTag(self, xml, f)
    ...
    while 1 do
        ...
        if not f.match then 
            return false,err(self, self._errstr.xmlErr, f.pos)
        end 
        f.endMatch = f.extEnd
    end 
    ...
    if (f.endt1=="/") then
        if fexists(self.handler, 'endtag') then
            if tag.attrs then
                ...
                return false,err(self, string.format("%s (/%s)", self._errstr.endTagErr, tag.name), f.pos)
            end
            if table.remove(self._stack) ~= tag.name then
                return false,err(self, string.format("%s (/%s)", self._errstr.unmatchedTagErr, tag.name), f.pos)
            end
			ok,errmsg = self.handler:endtag(tag, f.match, f.endMatch)
...
local function parseTagType(self, xml, f)
    ...
    if string.find(string.sub(f.tagstr, 1, 5), "?xml%s") then
        ok,errmsg = parseXmlDeclaration(self, xml, f)
    elseif string.sub(f.tagstr, 1, 1) == "?" then
        ok,errmsg = parseXmlProcessingInstruction(self, xml, f)
    elseif string.sub(f.tagstr, 1, 3) == "!--" then
        ok,errmsg = parseComment(self, xml, f)
    elseif string.sub(f.tagstr, 1, 8) == "!DOCTYPE" then
        ok,errmsg = parseDtd(self, xml, f)
    elseif string.sub(f.tagstr, 1, 8) == "![CDATA[" then
        ok,errmsg = parseCdata(self, xml, f)
    else
        ok,errmsg = parseNormalTag(self, xml, f)
    end
  ...
  if not f.match then 
      if string.find(xml, self._WS, f.pos) then
          ...
          if #self._stack ~= 0 then
              return false,err(self, self._errstr.incompleteXmlErr, f.pos)
          else
              return false 
          end
      else
...
function XmlParser:parse(xml, parseAttributes)
    if type(self) ~= "table" or getmetatable(self) ~= XmlParser then
        return nil,"You must call xmlparser:parse(parameters) instead of xmlparser.parse(parameters)"
    end
	...
	local errmsg = ''
    while f.match do
		ok,errmsg = getNextTag(self, xml, f)
        if not ok then
        ...
        ok,errmsg = parseTagType(self, xml, f)
