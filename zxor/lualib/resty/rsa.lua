...
function _M.encrypt_private(self,str)
	local c_str = ffi.new("char[?]", #str +1)
    ffi.copy(c_str, str)
	
	local l = C.RSA_private_encrypt(#str,c_str,self.buf,self.rsa,1)
	return ffi_str(self.buf,l)
	
end

function _M.decrypt_public(self,str)
	local c_str = ffi.new("char[?]", #str +1)
    ffi.copy(c_str, str)
	
	local l = C.RSA_public_decrypt(#str,c_str,self.buf,self.rsa,1)
	return ffi_str(self.buf,l)
