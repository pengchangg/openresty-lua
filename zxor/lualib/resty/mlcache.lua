...
opts-->
	lru	= lruobj,				--lru对象，可选
	lru_size = 100,				--worker中缓存的最大对象个数,默认100
	ttl = 30,					--缓存有效时间，单位秒，默认30秒,0表示不过期
	neg_ttl = 5,				--缓存中未命中对象有效时间，单位秒，默认5秒
	resurrect_ttl = 0.3,		--如果缓存从DB中获取失败，将在这个时间后恢复原先的值，可选
	shm_miss = '',				--未命中时，临时数据使用的缓存，可选
	shm_locks = '',				--锁定时，使用的缓存，默认为本身缓存
	shm_set_tries = 3,			--失败尝试次数，默认3
	resty_lock_opts = {},
	ipc_shm = '',				--ipc使用的share_dict的名字，与ipc互斥
	ipc_redis = '',				--ipc使用的redis的名字，与ipc互斥(优先使用ipc_shm)
	ipc = {						--自定义的ipc，与ipc_shm互斥
		subscribe = function(events) end,	--监听
		broadcast = function() end,					--广播
		poll = function() end,						--主动拉取
	},
	l1_serializer = function()	--缓存值序列化函数
		return ok,value
	end,
	debug = false,				--调试状态
--]]
...
function _M.new(name, shm, opts)
    ...
    if opts ~= nil then
		...
		elseif opts.ipc_redis ~= nil and type(opts.ipc_redis) ~= "string" then
            error("opts.ipc_redis must be a string", 2)
        end
		
        if opts.ipc ~= nil then
            if opts.ipc_shm or opts.ipc_redis then
                error("cannot specify both of (opts.ipc_shm or opts.ipc_redis) and opts.ipc", 2)
            end
            ...
            if type(opts.ipc.subscribe) ~= "function" then
                error("opts.ipc.subscribe must be a function", 2)
            end
    ...
    if opts.ipc_shm or opts.ipc_redis or opts.ipc then
        self.events = {
            ["invalidation"] = {
                channel = fmt("mlcache:invalidations:%s", name),
                handler = function(key,force)
                    self.lru:delete(key)
					if force then
        ...
        if opts.ipc_shm then
            local mlcache_ipc = require "resty.mlcache.ipc_shm"

            local ipc, err = mlcache_ipc.new(opts.ipc_shm, opts.debug, opts.lru_size)
            if not ipc then
                return nil, "failed to initialize mlcache IPC (could not instantiate mlcache.ipc): " .. err
            end
            ...
            self.poll = function(key, timeout)
                return ipc:poll(key, timeout)
            end

            self.ipc = ipc
		elseif opts.ipc_redis then
            local mlcache_ipc = require "resty.mlcache.ipc_redis"

            local ipc, err = mlcache_ipc.new(opts.ipc_redis)
            if not ipc then
                return nil, "failed to initialize mlcache IPC (could not instantiate mlcache.ipc_redis): " .. err
            end
            ...
            -- opts.ipc
			self.subscribe = function()
				local ok, err = opts.ipc.subscribe(self.events)
				if not ok and err ~= nil then
					return nil, "failed to initialize custom IPC (opts.ipc.subscribe returned an error): " .. err
				end
			end
...
function _M:set(key, opts, value)
    if not self.broadcast then
        error("no ipc to propagate update, specify opts.ipc_shm or opts.ipc_redis or opts.ipc", 2)
    end
...
function _M:delete(key)
    if not self.broadcast then
        error("no ipc to propagate deletion, specify opts.ipc_shm or opts.ipc_redis or opts.ipc",2)
    end
...
function _M:purge(flush_expired)
    if not self.broadcast then
        error("no ipc to propagate purge, specify opts.ipc_shm or opts.ipc_redis or opts.ipc", 2)
    end
...
function _M:update(key, timeout)
    if not self.poll then
        error("no polling configured, specify opts.ipc_shm or opts.ipc_redis or opts.ipc.poll", 2)
    end

    local ret, err = self.poll(key, timeout)
    if err then
